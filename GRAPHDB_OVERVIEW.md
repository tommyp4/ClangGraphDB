...
*   **Edges:**
    ...
    *   `(:Function)-[:USES_GLOBAL]->(:Global)`: **Planned.** Represents code reading/writing global state.
...
*   **Global State Tracking (`USES_GLOBAL` / `USES`):**
    *   **Current State:** Most parsers emit a generic `USES` edge for variable access.
    *   **Planned:** The `USES_GLOBAL` edge is explicitly used by the `globals` and `what-if` queries to track shared state between domains.

*   **Member Access:**
    *   Parsers extract Class Members as `Field` nodes. Accessing a field is modeled as a `USES` relationship, allowing dependency tracing through state, not just behavior.
