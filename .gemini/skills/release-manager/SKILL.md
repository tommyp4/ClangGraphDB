...
When the user requests to bump the version or prepare a release (e.g., "Prepare a new release v1.x.x"):

1.  **Verify State:** Ensure the working directory is clean (`git status`).
...
3.  **Summarize & Update Changelog:** 
    ...
    *   Prepend these notes into the `CHANGELOG.md` file under the new version header, following the existing format.
4.  **Commit & Tag:**
    *   Commit the updated changelog: `git commit -am "chore: release <version>"`
    *   Create a new Git tag: `git tag <version>` (e.g., `v1.0.0`).
